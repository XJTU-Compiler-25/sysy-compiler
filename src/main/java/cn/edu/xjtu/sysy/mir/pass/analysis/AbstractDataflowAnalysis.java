...
public abstract class AbstractDataflowAnalysis<Result> extends ModuleVisitor<Result> {
    ...
    public AbstractDataflowAnalysis(Pipeline<Module> pipeline, boolean isForward) {
        super(pipeline);
    ...
    public Set<BasicBlock> getPredBlocksOf(BasicBlock block) {
        return isForward ? CFGAnalysis.run(module).getPredBlocksOf(block) : CFGAnalysis.run(module).getSuccBlocksOf(block);
    }

    public Set<BasicBlock> getSuccBlocksOf(BasicBlock block) {
        return isForward ? CFGAnalysis.run(module).getSuccBlocksOf(block) : CFGAnalysis.run(module).getPredBlocksOf(block);
    }

    public List<BasicBlock> getOrderedBlocksOf(Function function) {
        return isForward ? CFGAnalysis.run(module).getRPOBlocks(function) : CFGAnalysis.run(module).getPOBlocks(function);
    }
    ...
    public void printResult(Function function) {
        for (var block : CFGAnalysis.run(module).getRPOBlocks(function)) {
